...
public class CompanyTestStudent {
    ...
    void getEmployeesByPredicate() {
        int femaleCount = testCompany.getEmployeesByPredicate(Employee::isFemale).size();
        int maleCount = testCompany.getEmployeesByPredicate(Employee -> !Employee.isFemale()).size();
        assertEquals(testCompany.getAllEmployees().size(), femaleCount + maleCount);
    }
