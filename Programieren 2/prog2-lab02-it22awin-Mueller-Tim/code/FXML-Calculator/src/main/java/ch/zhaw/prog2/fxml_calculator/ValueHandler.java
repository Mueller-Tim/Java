...
import static ch.zhaw.prog2.fxml_calculator.ValueHandler.ValuesState.*;
...
public class ValueHandler {
    ...
    private final SimpleObjectProperty<ValuesState> valuesState = new SimpleObjectProperty(UNDEFINED);

    /** Enum to store the validity state of the form values */
    enum ValuesState {
        UNDEFINED, OK, ERROR, HELP
    }

    /** returns the validation state of the form values */
    public SimpleObjectProperty<ValuesState> getValuesState() {
        return this.valuesState;
    }

    /** Reset the values state to the default value (UNDEFINED) */
    public void resetValuesState() {
        this.valuesState.setValue(UNDEFINED);
    }
    ...
    public void validateAndSetValues(String initialAmount, String returnInPercent, String annualCost, String numberOfYears) {
        ...
        this.numberOfYears = parseNumberValue(numberOfYears, 1, 99, "Please enter a time period 1 to 99 years!", errorMessage)
            .orElse(0).intValue();
        setResult(this.valuesState.getValue() == OK ? calculateResult() : errorMessage.toString());
    }
    ...
    private Optional<Number> parseNumberValue(String numberString, Number minValue, Number maxValue,
        ...
        this.valuesState.setValue(ERROR);
    ...
    public void showHelp(){
        valuesState.setValue(HELP);
        this.setResult("Enter valid values to\n " +
            "- Initial amount (>0)\n " +
            "- Return in % (can be +/- or 0)\n" +
            "- Annual Costs (>0) \n" +
            "- Number of years (>0) \n\n" +
            "\"Calculate\" displays the annual balance development.\n" +
            "\"Open Result\" opens an extra window for the result.");
    }
